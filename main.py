...
def run():
    # Render automatically uses port 10000 or 8080
...
insquad = None 
joining_team = False 
team_chat_id = None  # Last known team chat ID 
online_writer = None 
whisper_writer = None 
...
ADMIN_UID = "2439692866"
server2 = "BD"
key2 = "mg24"
BYPASS_TOKEN = "your_bypass_token_here"
WHITELIST_ONLY = False
bot_enabled = True
BOT_OWNER_UID = 2436392866  
PLAYER_NAME_CACHE = {}
...
def start_api_servers():
    ...
    if not os.path.exists(apis_dir) or not os.path.isdir(apis_dir):
        ...
        fields = {
            ...
            2: {
                ...
                27: {                    # Field 27 (critical for badge!)
                    1: 11,               # Field 27.1
                    2: 14423196098,      # Field 27.2 (your bot UID)
                    3: 9999              # Field 27.3
                },
                28: {},                   # Empty field 28
...
async def black666(client_id, key, iv):
    banner_text = "[FF0000][B][C] ERROR , WELCOME TO [FFFFFF] PRIME CITY [00FF00] GULID BOT  \n[FFFF00]NEW VERSION NEW FUNCTION !\n[FF0000]TELEGRAM : @Shareyabha8\n\n"     
    fields = {
        1: 5,
        2: {
            1: int(client_id),
...
async def lag_team_loop(team_code, key, iv, region):
    """Ultra aggressive rapid join/leave loop to create maximum lag"""
    global lag_running
    count = 0

    while lag_running:
        try:
            # Burst: 5 join/leave packets in rapid succession (no delay)
            for _ in range(5):
            ...
            count += 1
            print(f"💥 LAG burst #{count} on team: {team_code}")
            await asyncio.sleep(0.005)  # 5ms between bursts - maximum pressure
...
[C][B][FFFFFF] Developer :—͞ᎮC ᎡØᎷÑƇᏆ 
[C][B][00FFFF]╚═══━━━✦❘༻༺❘✦━━━═══╝
"""
    # Send main info
...
[C][B][11EAFD╚═══━━━✦❘༻༺❘✦━━━═══╝
[C][B][FFB300] Telegram: [FFFFFF]KING_DULA_BOT[00FF00]!!
"""
...
[FFFFFF]Likes After : [00FF00]{xMsGFixinG(likes_after)}  
[C][B][11EAFD]‎╚═══━━━✦❘༻༺❘✦━━━═══╝
[C][B][FFB300] DEVELOPER: [FFFFFF]SHAREYA [00FF00]!!
"""
        elif status == 2 or likes_before == likes_after:
            # 🚫 Already claimed / Maxed
            return f"""
...
[00FF00]—͞ᎮC ᎡØᎷÑƇᏆ  TCP
WELCOME TO PRIME CITY GUILD BOT
...
async def send_all_titles_sequentiallly(uid, chat_id, key, iv, region, chat_type):
    ...
    # Send initial message
    start_msg = f"""[B][C][00FF00] Noobde —͞ᎮC ᎡØᎷÑƇᏆ  ya meku agar tu noob bolra toh tu g a y hai
              ...
              # =================== EMOTE DETECTION ONLY ===================
                    ...
                    try:
                        ...
                        if "MĢ24_GÀMER" in WHITELISTED_UIDS:
                            ...
                            try:
                                ...
                                bot_uid = 14423196098
                                message_text = f" Can't accept Your request Talk to —͞ᎮC ᎡØᎷÑƇᏆ "
                ...
                if insquad == True:
                    try:
                        ...
                        message = """[B][C][FF0000]WELCOME TO PRIME CITY GUILD BOT
[FFFF00]╔══━━✦❘༻༺❘✦━━═══╗
[FFFFFF]⚡ BOT POWERED BY:[FF0000] KING DULA 
[FFFFFF]🎯 STATUS: [00FF00]ACTIVE 24/7
[FFFFFF]📱 CONTACT: [FFFF00]KING_DULA_BOT
[00FF00]»»————-　★　————-««
[FFFF00]╚══━━✦❘༻༺❘✦━━═══╝"""
                ...
                if senthi == True:
                    ...
                    def get_random_color(): return "_" 
                    message = """[B][C][FF0000] ∆WELCOME TO KING DULA BOT∆
[00FF00]╔══━━✦❘༻༺❘✦━━═══╗
[FFFFFF]⚡ Admin by :[FF0000] DULA 
            ...
            while True:
                ...
                if data.hex().startswith("120000"):
                    ...
                    if response:
                        ...
                        # Team chat_id auto-save (chat_type=0 মানে Team/Squad)
                        if response.Data.chat_type == 0:
                            team_chat_id = chat_id
                            print(f"💾 Team chat_id saved: {team_chat_id}")
...
# GUILD INFORMATION FF
                        ...
                        if inPuTMsG.strip().startswith('/wlremove'):
                            ...
                            # Check owner
                            if str(response.Data.uid) != "2439692866":
...
"""
                            ...
                            if str(response.Data.uid) != "2439692866":  # Replace with your actual UID
                            ...
                            # Send result
                            if success:
                                bot_uid = 14423196098 #Not esy2nd id
...
"""
                        ...
                        if inPuTMsG.strip() == '/wllist':
                            ...
                            # Check if owner
                            if str(response.Data.uid) != "2439692866":  # Your UID
...
👥 Whitelisted UIDs:"""
                            ...
                            for uid in WHITELISTED_UIDS:
                                if uid != "2439692866":  # Skip owner since already shown
                                    whitelist_msg += f"\n• {xMsGFixinG(uid)}"
                                    count += 1
                                    if count >= 20:
...
"""
                        ...
                        if inPuTMsG.strip() == "/admin":
                            # Process /admin command - PRIME CITY GUILD Admin Info
                            admin_message = """
[B][C][FFD700]╔══━━✦❘༻༺❘✦━━═══╗
[B][C][FF4500]👑 PRIME CITY GUILD ADMIN 👑
[B][C][FFD700]╚══━━✦❘༻༺❘✦━━═══╝
[B][C][00FF00]Admin Name  : [FFFF00]SHAREYA ISLAM

[B][C][00BFFF]📱 WhatsApp  : [FFFFFF]+8801307874600
[B][C][00BFFF]📲 Telegram  : [FFFFFF]KING_DULA_BOT
[B][C][00BFFF]💬 Discord   : [FFFFFF]Knox by Shareya

[B][C][FF4444]⚠️ Please do NOT call Admin directly!
[B][C][FFFF00]For any issue, message on WhatsApp only.

[B][C][FF0000] ✦══━━✦❘༻༺❘✦━━═══✦
[B][C][00FF00]🤖 Bot by PRIME CITY GUILD
"""
...
# GLOBAL BLOCKED NAMES
                        BLOCKED_NAMES = ["maruf", "hexozenta", "—͞ᎮƇ ᎡØᎷÑƇᏆ", "hexo-king", "hexo_king", "hexoking", "hexo king"]  # Protected names

                        # GALi / JOKE MESSAGE
                        if inPuTMsG.strip().startswith('/gali'):
                            print('Processing /gali command')
                            # Team chat_id check
                            target_team_chat = team_chat_id if team_chat_id else (chat_id if response.Data.chat_type == 0 else None)
                            if not target_team_chat:
                                await safe_send_message(response.Data.chat_type, "[B][C][FF0000]❌ Bot কোনো Team-এ নেই! আগে Team-এ join করুন।", uid, chat_id, key, iv)
                                continue
                            ...
                            try:
                                ...
                                messages = [
                                    ...
                                    "{Name} মাদার চো🤫দ, তোর মা🤫কে ১৮০ কি.মি. স্পিডে চু🤫দি তেলের দাম বেশি তাই ছেপ লাগাইয়া চু***!!",
                                ...
                                for msg in messages:
                                    ...
                                    await safe_send_message(0, colored_message, uid, send_chat_id, key, iv)
                                    await asyncio.sleep(0.5)
...
# PRAISA COMMAND (17 POSITIVE MESSAGES)
                        ...
                        if inPuTMsG.strip().startswith('/roast '):
                            ...
                            try:
                                ...
                                blocked = any(b.lower() in name_lower for b in BLOCKED_NAMES)
                                if blocked:
                                    block_msg = "[B][C][FF0000]❌ এই নাম protected! অন্য নাম দাও।"
                                    await safe_send_message(response.Data.chat_type, block_msg, uid, chat_id, key, iv)
                                    continue

                                import random
                                roast_messages = [
                                    "🔥 {Name} তোর IQ এতটাই কম যে, Calculator-ও তোকে দেখে হাসে! 😂",
                                    "💀 {Name} তুই Free Fire খেলিস? নাকি শুধু মরতে আসিস? 🤣",
                                    "😂 {Name} এর aim এতটাই খারাপ, সে নিজের পায়ে গুলি করে! 🦶",
                                    "🤡 {Name} তোর rank দেখে বুঝলাম, তুই Lobby-তে বসে থাকাই শিখছিস! 😭",
                                    "💩 {Name} তোর gameplay দেখে আমার মনে হলো, তুই keyboard দিয়ে খেলছিস না, মাথা দিয়ে! 🤦",
                                    "😹 {Name} তোর squad তোকে দেখলে নিজেরাই সরেন্ডার করে! 🏳️",
                                    "🗑️ {Name} তুই Booyah পাওয়ার চেয়ে লুডু খেলাই ভালো করতিস! 🎲",
                                    "🔥 {Name} তোর headshot rate এতটাই কম যে, দেওয়ালেও গুলি লাগে না! 🧱",
                                    "😂 {Name} তোকে দেখলে enemy-রা হাসতে হাসতে মরে যায়! হেডশটের দরকারই হয় না! 💀",
                                    "🤣 {Name} তোর Free Fire career দেখে মনে হয় তুই চোখ বন্ধ করে খেলছিস! 😎",
                                    "💀 {Name} তুই নাকি গেমার? আমার দাদুও তোর চেয়ে ভালো খেলবে! 👴",
                                    "😂 {Name} তোর প্রতিটা match শেষ হয় একটাই ভাবে — ব্যাগ নিয়ে বাড়ি ফেরা! 🎒",
                                    "🤡 {Name} তোর গেমিং চেয়ার কাঁদছে, কারণ সে জানে তুই কতটা খারাপ! 😭",
                                    "🔥 {Name} তোর kill count দেখে মনে হলো তুই শুধু spectate করতে আসিস! 👀",
                                    "💩 {Name} তুই যে টিমে থাকিস, সে টিম আগে থেকেই হেরে যায়! 🏳️",
                                    "😹 {Name} তোর হাতের লক্ষ্য এতটাই খারাপ যে, গোলা ছাড়লে নিজের মাথায় পড়ে! 🤦",
                                    "🗑️ {Name} তুই Noob না, তুই Noob-এর শিক্ষক! 📚",
                                ]

                                selected = random.sample(roast_messages, min(5, len(roast_messages)))

                                # Intro message - Team chat এ পাঠাবে
                                intro = f"[B][C][FF4500]🔥🔥 {name.upper()} কে ROAST শুরু হচ্ছে! 🔥🔥"
                                await safe_send_message(0, intro, uid, target_team_chat, key, iv)
                                await asyncio.sleep(1)

                                colors = ["FF4500", "FF6347", "FF0000", "FF8C00", "FFD700"]
                                for i, msg in enumerate(selected):
                                    color = colors[i % len(colors)]
                                    colored_message = f"[B][C][{color}] {msg.replace('{Name}', name.title())}"
                                    await safe_send_message(0, colored_message, uid, target_team_chat, key, iv)
                                    await asyncio.sleep(1.5)

                                # Outro - Team chat এ পাঠাবে
                                outro = f"[B][C][FF4500]💀 {name.upper()} ROASTED! 🔥 PRIME CITY GUILD BOT 💀"
                                await safe_send_message(0, outro, uid, target_team_chat, key, iv)

                            except Exception as e:
                                error_msg = f"[B][C][FF0000]❌ ERROR: {str(e)}"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)

                        # /love command - শুধু team chat-এ
    ...
    "{Name} I LOVE YOU JAN ♡"
                                    ...
                                    # Send each message - Team chat এ পাঠাবে
                                    for msg in messages:
                        ...
                        # ══════════════════════════════════════
                        # 💥 /lag COMMAND - ULTRA LAG ATTACK
                        # Usage: /lag <teamcode> <time>
                        # Time options: 15 30 60 120
                        # ══════════════════════════════════════
                        if inPuTMsG.strip().startswith('/lag '):
                            print('Processing /lag command')
                            parts = inPuTMsG.strip().split()

                            ALLOWED_TIMES = [15, 30, 60, 120]

                            if len(parts) < 3:
                                usage_msg = (
                                    "[B][C][FF4500]❌ সঠিক format নয়!\n"
                                    "[FFFF00]Usage: /lag (teamcode) (time)\n"
                                    "[FFFFFF]Example: /lag 54686 30\n"
                                    "[00FFFF]Allowed time: 15s | 30s | 60s | 120s"
                                )
                                await safe_send_message(response.Data.chat_type, usage_msg, uid, chat_id, key, iv)
                                continue

                            lag_teamcode = parts[1].strip()
                            try:
                                lag_time = int(parts[2].strip())
                            except ValueError:
                                await safe_send_message(response.Data.chat_type, "[B][C][FF0000]❌ Time অবশ্যই number হতে হবে! (15/30/60/120)", uid, chat_id, key, iv)
                                continue

                            if lag_time not in ALLOWED_TIMES:
                                await safe_send_message(response.Data.chat_type, "[B][C][FF0000]❌ Invalid time!\n[FFFF00]Allowed: 15 | 30 | 60 | 120 seconds only", uid, chat_id, key, iv)
                                continue
                            ...
                            start_msg = (
                                f"[B][C][FF4500]💥━━━━━━━━━━━━━━━━━━💥\n"
                                f"[FFFF00]  ⚡ LAG ATTACK STARTED ⚡\n"
                                f"[FF4500]💥━━━━━━━━━━━━━━━━━━💥\n"
                                f"[FFFFFF]🎯 Team Code : [00FFFF]{lag_teamcode}\n"
                                f"[FFFFFF]⏱️ Duration  : [FF4500]{lag_time} seconds\n"
                                f"[FFFFFF]🔥 Mode      : [FF0000]ULTRA BURST\n"
                                f"[FF4500]💥━━━━━━━━━━━━━━━━━━💥"
                            )
                            await safe_send_message(response.Data.chat_type, start_msg, uid, chat_id, key, iv)

                            async def run_lag_with_timer(tc, duration, ct, cid):
                                global lag_running, lag_task
                                try:
                                    task = asyncio.create_task(lag_team_loop(tc, key, iv, region))
                                    lag_task = task
                                    ...
                                    stop_msg = (
                                        f"[B][C][00FF00]✅━━━━━━━━━━━━━━━━━━✅\n"
                                        f"[FFFF00]  ⚡ LAG ATTACK FINISHED ⚡\n"
                                        f"[00FF00]✅━━━━━━━━━━━━━━━━━━✅\n"
                                        f"[FFFFFF]🎯 Team : [00FFFF]{tc}\n"
                                        f"[FFFFFF]✅ Done : [00FF00]{duration}s completed\n"
                                        f"[00FF00]✅━━━━━━━━━━━━━━━━━━✅"
                                    )
                                    await safe_send_message(ct, stop_msg, uid, cid, key, iv)
                                except asyncio.CancelledError:
                                    lag_running = False

                            asyncio.create_task(run_lag_with_timer(
                                lag_teamcode, lag_time,
                                response.Data.chat_type, chat_id
                            ))

                        # ══════════════════════════════════════
                        # 🛑 /stoplag COMMAND
                        # ══════════════════════════════════════
                        if inPuTMsG.strip() == '/stoplag':
                            if lag_task and not lag_task.done():
                                lag_running = False
                                lag_task.cancel()
                                stop_msg = "[B][C][00FF00]✅ LAG ATTACK বন্ধ করা হয়েছে!"
                            else:
                                stop_msg = "[B][C][FFFF00]⚠️ কোনো lag চলছে না!"
                            await safe_send_message(response.Data.chat_type, stop_msg, uid, chat_id, key, iv)

                        # ══════════════════════════════════════
                        # 🤖 /bot tc COMMAND - Admin Only
                        # Bot যে group-এ আছে সেই team code admin কে পাঠাবে
                        # ══════════════════════════════════════
                        if inPuTMsG.strip() == '/bot tc':
                            if not is_admin(uid):
                                await safe_send_message(response.Data.chat_type, "[B][C][FF0000]❌ শুধু Admin এই command ব্যবহার করতে পারবে!", uid, chat_id, key, iv)
                                continue

                            try:
                                current_tc = str(chat_id)

                                # Admin কে private message পাঠাও
                                tc_msg = (
                                    f"[B][C][00FFFF]╔╔═══━━✦❘༻༺❘✦━━═══╗\n"
                                    f"[FFFF00]  🤖 BOT TEAM CODE INFO\n"
                                    f"[00FFFF]╚═══━━✦❘༻༺❘✦━━═══╝\n"
                                    f"[FFFFFF]📌 Team Code : [00FF00]{current_tc}\n"
                                    f"[FFFFFF]💬 Chat Type : [FFFF00]{response.Data.chat_type}\n"
                                    f"[FFFFFF]👤 Requested: [00FFFF]{uid}\n"
                                    f"[00FFFF]╚═══━━✦❘༻༺❘✦━━═══╝\n"
                                    f"[FF00FF]⚡ PRIME CITY GUILD BOT"
                                )

                                # Team chat-এও confirm দাও
                                confirm_msg = f"[B][C][00FF00]✅ Team Code আপনার কাছে পাঠানো হয়েছে! TC: [FFFF00]{current_tc}"
                                await safe_send_message(response.Data.chat_type, confirm_msg, uid, chat_id, key, iv)

                                # Admin এর private chat-এ পাঠাও (chat_type=2, uid=ADMIN_UID)
                                admin_uid_int = int(ADMIN_UID)
                                private_packet = await xSEndMsg(tc_msg, 2, admin_uid_int, admin_uid_int, key, iv)
                                await SEndPacKeT(whisper_writer, online_writer, 'ChaT', private_packet)
                                print(f"✅ Team code {current_tc} sent to admin {ADMIN_UID}")
...
║ ══━━✦❘༻༺❘✦━━══
║                                       
║ ⚡ [FF00FF] PRIME CITY GUILD VIP BOT ⚡   
╚══━━✦❘༻༺❘✦━━═══╝"""
                            await safe_send_message(response.Data.chat_type, admin_message, uid, chat_id, key, iv)
...
# In TcPChaT function, update /status command:
                        ...
                        #==================≈===========  /HELP MENU COMMANDS ========================================
                        # ========= GUILD RULES COMMAND =========
                        if inPuTMsG.strip().lower() in ("/rules", "rules"):
                            print('Processing /rules command')
                            ...
                            rules_messages = [
                                ...
                                f"[B][C][FFFF00]{N} Whatsapp গ্রুপটি শুধুমাত্র Prime City Guild সংক্রান্ত আলোচনা ও তথ্যের জন্য। নিয়ম অমান্য করলে গ্রুপ থেকে Remove করা হতে পারে। ভুলবশত কোনো message গেলে দ্রুত Delete করুন। সবার সহযোগিতার জন্য ধন্যবাদ 🙏✨",
                                f"[B][C][FF0000]{N} নিষেধাজ্ঞা: অজানা Link বা Video share করবেন না। QR কোড post করবেন না। Personal Info শেয়ার করবেন না। Promote চাইলে Admin এর সাথে যোগাযোগ করুন!!!",
                                f"[B][C][FF8C00]{N} আপনি এবং Guild এর সকল player দের জন্য সতর্কবার্তা: 📢 গিল্ড Level 7 থেকে Level 6 এ নামার কারণে নতুন কঠোর নিয়ম চালু হয়েছে। প্রতিদিনের Glory Target পূরণ করতে হবে। নিয়মিত active না থাকলে বা target miss করলে notice ছাড়াই Kick করা হবে। আমাদের লক্ষ্য আবার Level 7 এ ফেরা! Admin: Shareya — by PRIME CITY GUILD",
                            ]

                            for rule_msg in rules_messages:
                                await safe_send_message(response.Data.chat_type, rule_msg, uid, chat_id, key, iv)
                                await asyncio.sleep(0.3)

                        if inPuTMsG.strip().lower() in ("help", "/help", "menu", "/menu", "5", "bot", "list", "/list", "show", "hi"):
                            ...
                            # ════════════════════════════════
                            header = f"""[B][C][FF00FF]╔══━━✦❘༻༺❘✦━━═══╗
[FFFF00]  ⚡ KING_DULA_BOT ⚡
[00FFFF]  🤖 Version : KING_DULA_BOT_V4 
[FF00FF]◈━━━━━━━━━━━━━━◈
...
[FF00FF]╚══━━✦❘༻༺❘✦━━═══╝"""
                            ...
                            # ════ PAGE 1 : INFO & BASIC ════
                            help_1 = """[B][C][00FFFF]╔══━━✦❘༻༺❘✦━━═══╗
[FFFF00]  📊 INFO & BASIC COMMANDS
[00FFFF]╚══━━✦❘༻༺❘✦━━═══╝
[FF4500]❶[FFFFFF] Get Likes    [00FF00]» /like [uid]
[FF4500]❷[FFFFFF] Player Info  [00CCFF]» /info [uid]
[FF4500]❸[FFFFFF] Check Ban    [00FF00]» /check [uid]
[FF4500]❹[FFFFFF] Player Inv   [00CCFF]» /inv [uid]
[FF4500]❺[FFFFFF] Group Limit  [FF0000]» /3 /5 /6
[FF4500]❻[FFFFFF] Join Team    [FFFF00]» /join [TC]
[FF4500]❼[FFFFFF] Leave Group  [FF0000]» /exit
[FF4500]❽[FFFFFF] Admin Info   [00CCFF]» /admin
[FF4500]❾[FFFFFF] Bot Status   [00FF00]» /status [uid]
[FF4500]❿[FFFFFF] AI Chat      [FF00FF]» /ai [msg]
[00FFFF]╚══━━✦❘༻༺❘✦━━═══╝"""
                            ...
                            # ════ PAGE 2 : SPAM & ATTACK ════
                            help_2 = """[B][C][FF4500]╔══━━✦❘༻༺❘✦━━═══╗
[FFFF00]  💥 SPAM & ATTACK COMMANDS
[FF4500]╚══━━✦❘༻༺❘✦━━═══╝
[FF00FF]⓫[FFFFFF] Msg Spam     [00FF00]» /ms [msg]
[FF00FF]⓬[FFFFFF] COPY Emote  [00FF00]» upcoming [uid]
[FF00FF]⓭[FFFFFF] Emote lock  [00CCFF]» upcoming [uid]
[FF00FF]⓮[FFFFFF] Hijack [FFFF00]» /upcoming [uid]
[FF00FF]⓯[FFFFFF] Join Spam    [FF00FF]» /spam_join [uid]
[FF00FF]⓰[FFFFFF] Kick Player  [FF6600]» /kick [uid]
[FF00FF]⓱[FFFFFF] Force Start  [FFCC00]» /start [TC]
[FF00FF]⓲[FFFFFF] Add Friend   [00FF00]» /add [uid]
[FF00FF]⓳[FFFFFF] Remove Frnd  [FF00FF]» /remove [uid]
[FF00FF]⓴[FFFFFF] Unlock 5v5   [FFFF00]» /snd [uid]
[FF4500]╚══━━✦❘༻༺❘✦━━═══╝"""
                            ...
                            # ════ PAGE 3 : LAG & ATTACK ════
                            help_3 = """[B][C][FF0000]╔══━━✦❘༻༺❘✦━━═══╗
[FFFF00]  ⚡ LAG & ATTACK COMMANDS
[FF0000╚══━━✦❘༻༺❘✦━━═══╝
[00FF00]㉑[FFFFFF] Lag Attack   [FF4500]» /lag [TC] [15/30/60/120]
[00FF00]㉒[FFFFFF] Stop Lag     [FF0000]» /stoplag
[00FF00]㉓[FFFFFF] Ghost Join   [00FFFF]» /ghost [TC]
[00FF00]㉔[FFFFFF] Bundle Spam  [00FF00]» /bb_lag [TC]
[00FF00]㉕[FFFFFF] Attack Team  [FF0000]» /attack [TC]
[00FF00]㉖[FFFFFF] Level Up Bot [FFFF00]» /lw [TC]
[00FF00]㉗[FFFFFF] Stop LvUp    [FF0000]» /stop
[00FF00]㉘[FFFFFF] Bot TC Info  [00FFFF]» /bot tc
[00FF00]㉙[FFFFFF] Block User   [00FFFF]» /block [uid]
[00FF00]㉚[FFFFFF] Unblock User [00FF00]» /unblock [uid]
[FF0000]╚══━━✦❘༻༺❘✦━━═══╝"""
...
[CC33FF]╚══━━✦❘༻༺❘✦━━═══╝
[00FFFF]㉛[FFFFFF] Emote Play   [00FF00]» /play [uid] [1-410]
[00FFFF]㉜[FFFFFF] Emote Cmd    [00CCFF]» /emote [U1][U2][TC][No]
[00FFFF]㉝[FFFFFF] EVO Emote    [00FF00]» /evo [U1][U2][No]
[00FFFF]㉞[FFFFFF] Play (ID)    [CC33FF]» /e [U1][U2][ID]
[00FFFF]㉟[FFFFFF] Fast Play    [00FF00]» /fast [U1][U2][ID]
[00FFFF]㊱[FFFFFF] Times Emote  [FF0000]» /p [uid][emote][count]
[00FFFF]㊲[FFFFFF] EVO Cycle    [00CCFF]» /evos [uid]
[00FFFF]㊳[FFFFFF] Stop EVO     [00FFFF]» /sevos
[00FFFF]㊴[FFFFFF] Self Emote   [FF00FF]» @evos
[00FFFF]㊵[FFFFFF] Emote w/o B [FF6600]» /me [TC][emote]
[CC33FF]╚══━━✦❘༻༺❘✦━━═══╝"""
                            ...
                            # ════ PAGE 5 : BADGE & FUN ════
                            help_5 = """[B][C][FFFF00]╔══━━✦❘༻༺❘✦━━═══╗
[FFFF00]  🏅 BADGE & FUN COMMANDS
[FFFF00]||══━━✦❘༻༺❘✦━━═══||
[FF1493]㊶[FFFFFF] CL Badge  [FF0000]» /s1 [uid]
[FF1493]㊷[FFFFFF] V Badge      [00FF00]» /s2 [uid]
[FF1493]㊸[FFFFFF] Mdtr Badge   [0088FF]» /s3 [uid]
[FF1493]㊹[FFFFFF] Old V Badge  [FFFF00]» /s4 [uid]
[FF1493]㊺[FFFFFF] Pro Badge    [00AAFF]» /s5 [uid]
[FF1493]㊻[FFFFFF] Bundle Info  [00FF00]» /bundle [name]
[FF1493]㊼[FFFFFF] TCP Store    [FF00FF]» /store
[FF1493]㊻[FFFFFF] GULID Rules  [00AAFF]» /rules 
[FF1493]㊽[FFFFFF] Roast Name   [FF4500]» /roast [name]
[FF1493]㊾[FFFFFF] Love Msg     [FF69B4]» /love [name]
[FF1493]㊿[FFFFFF] Gali Msg   [FF0000]» /gali [name]
[FFFF00]╚══━━✦❘༻༺❘✦━━═══╝"""
                            ...
                            # ════ PAGE 6 : ADMIN ════
                            help_6 = """[B][C][FF6600]╔══━━✦❘༻༺❘✦━━═══╗
[FFFF00]  👑 ADMIN ONLY COMMANDS
[FF6600]╚══━━✦❘༻༺❘✦━━═══╝
[00FF00]🔒[FFFFFF] Bot ON       [00FF00]» /on
[00FF00]🔒[FFFFFF] Bot OFF      [FF0000]» /off
[00FF00]🔒[FFFFFF] Admin Mode   [00CCFF]» /adon
[00FF00]🔒[FFFFFF] Admin Off    [FFFF00]» /adoff
[00FF00]🔒[FFFFFF] DM Message   [FFFF00]» /dm [msg]
[00FF00]🔒[FFFFFF] Bot TC       [00FFFF]» /bot tc
[00FF00]🔒[FFFFFF] GULID Rules  [FF00FF]» /rules
[FF6600]||══━━✦❘༻༺❘✦━━═══||
[FFFF00]⚠️ [FF0000]Admin commands are
[FFFF00]   restricted to owner only!
[FF6600]╚══━━✦❘༻༺❘✦━━═══╝"""
...
[B][C][FF00FF]✦══━━━✦❘༻༺❘✦━━━══✦
[FFFFFF]👤 Dev     [FF1493]» KING_DULA
[FFFFFF]💬 WA      [00FF00]» +94719251323
[FFFFFF]📱 TG      [00CCFF]» KING_DULA_BOT
[FFFFFF]🌟 Guild   [FFFF00]» KING_DULA_BOT
[FFFFFF]⚡ Version [00FFFF]» KING_DULA_BOT_V4
[B][C][FF00FF]╔══━━✦❘༻༺❘✦━━═══╗
...
if __name__ == '__main__':
    # Flask web server চালু করুন (Render alive রাখার জন্য)
    print("🚀 Starting Web Server for Render/UptimeRobot...")
    keep_alive()
